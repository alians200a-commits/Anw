...
export default function AdminAppV3() {
  ...
  const filtered = useMemo(() => {
    ...
    return content.filter((item) => {
      ...
      if (!q) return true;
      return [item.slug, item.title_ar ?? '', item.title_en ?? '', STATUS_LABELS[item.status]]
        .some((value) => value.toLowerCase().includes(q));
    });
  }, [content, query, statusFilter]);

  const countForFilter = (id: StatusFilter) => id === 'all' ? content.length : counts[id];
    ...
    <div dir="rtl" className="admin-mobile-select-scope min-h-screen overflow-x-hidden bg-[#eef3f8] text-[#24313f]">
      <header className="border-b border-white/10 bg-[#07182c] text-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-4 sm:px-6 sm:py-5">
          <div className="min-w-0"><p className="text-xs font-bold text-[#d9a441]">دليلي — مملكة التخدير</p><h1 className="mt-1 truncate text-lg font-black sm:text-xl">لوحة إدارة المحتوى</h1></div>
      ...
      <main className="mx-auto max-w-6xl space-y-4 px-3 py-4 sm:space-y-6 sm:px-6 sm:py-6">
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/70 sm:rounded-3xl sm:p-5"><Users className="mb-3 text-[#2f69a8]" size={23} /><div className="text-2xl font-black text-[#0a2037] sm:text-3xl">{canManageTeam ? invites.length : '—'}</div><div className="mt-1 text-xs font-bold text-slate-500 sm:text-sm">فريق الإدارة</div></div>
        </section>
            ...
            {FILTERS.map((filter) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex min-h-10 shrink-0 items-center gap-2 rounded-full px-3.5 text-xs font-black ring-1 transition ${active ? 'bg-[#173a63] text-white ring-[#173a63]' : 'bg-white text-[#526675] ring-[#d9e6f2]'}`}
                >
                  <span>{filter.label}</span>
                  <span className={`rounded-full px-1.5 py-0.5 text-[10px] ${active ? 'bg-white/15 text-white' : 'bg-[#eef3f8] text-[#526675]'}`}>{countForFilter(filter.id)}</span>
                </button>
              );
            })}
          </div>
        ...
        {canManageTeam && (
          <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/70 sm:rounded-3xl sm:p-6">
            <div className="mb-4 flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#eef3f8] text-[#173a63]"><UserPlus size={21} /></div><div className="min-w-0"><h2 className="font-black text-[#0a2037]">إضافة شخص لفريق الإدارة</h2><p className="text-xs text-slate-500 sm:text-sm">كل شخص بحسابه وصلاحيته الخاصة.</p></div></div>
            <form onSubmit={inviteUser} className="grid gap-3 md:grid-cols-[1fr_180px_auto]">
              <input type="email" dir="ltr" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="editor@example.com" className="min-h-12 min-w-0 rounded-2xl border border-[#d9e6f2] px-4 py-3 text-base outline-none focus:border-[#2f69a8]" />
              <select aria-label="صلاحية الشخص" value={inviteRole} onChange={(event) => setInviteRole(event.target.value as AdminRole)} className="min-h-12 w-full rounded-2xl border border-[#d9e6f2] bg-white px-4 py-3 text-base font-bold outline-none"><option value="editor">محرر محتوى</option><option value="reviewer">مراجع</option>{profile.role === 'owner' && <option value="admin">مدير</option>}</select>
              <button type="submit" disabled={inviteBusy} className="min-h-12 rounded-2xl bg-[#173a63] px-5 py-3 font-black text-white disabled:opacity-60">{inviteBusy ? 'جاري…' : 'إضافة الدعوة'}</button>
            </form>

            {invites.length > 0 && (
              <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {invites.map((invite) => (
                  <div key={invite.id} className="min-w-0 rounded-2xl border border-slate-100 bg-[#f8fbfd] p-3.5">
                    <div dir="ltr" className="truncate text-left text-sm font-bold text-[#0a2037]">{invite.email}</div>
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                      <span className="rounded-full bg-white px-2.5 py-1 font-bold text-[#173a63] ring-1 ring-[#d9e6f2]">{ROLE_LABELS[invite.role]}</span>
                      <span className="text-slate-500">{invite.accepted_at ? 'مفعّل' : invite.is_active ? 'بانتظار التفعيل' : 'متوقف'}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/70 sm:rounded-3xl sm:p-6">
          <div className="mb-4 flex items-end justify-between gap-3">
            <div><h2 className="font-black text-[#0a2037]">المحتوى</h2><p className="mt-1 text-xs text-slate-500 sm:text-sm">النتائج الحالية: {filtered.length}</p></div>
            {(query || statusFilter !== 'all') && <button type="button" onClick={() => { setQuery(''); setStatusFilter('all'); }} className="shrink-0 rounded-xl bg-[#eef3f8] px-3 py-2 text-xs font-black text-[#173a63]">مسح الفلاتر</button>}
          </div>

          {filtered.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#b9cee2] bg-[#f8fbfd] p-8 text-center"><Pill className="mx-auto mb-3 text-[#2f69a8]" size={28} /><p className="text-sm font-semibold text-slate-500">لا توجد نتائج مطابقة.</p></div>
          ) : (
            <div className="space-y-2">
              {filtered.map((item) => (
                <article key={item.id} className="rounded-2xl border border-slate-100 bg-white p-3.5 sm:p-4">
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${STATUS_CLASSES[item.status]}`}>{STATUS_LABELS[item.status]}</span>
                    <div className="min-w-0 flex-1 text-right">
                      <div className="break-words font-black text-[#0a2037]">{item.title_ar || item.title_en || item.slug}</div>
                      <div className="mt-1 truncate text-xs text-slate-400" dir="ltr">{item.title_en || item.slug} · v{item.version}</div>
                    </div>
                  </div>

                  <div className="mt-3 flex flex-wrap gap-2 border-t border-slate-100 pt-3">
                    {item.content_type === 'drug' && canEdit && <a href={`/admin/drugs?id=${encodeURIComponent(item.id)}`} className="inline-flex min-h-10 flex-1 items-center justify-center gap-1 rounded-xl border border-[#d9e6f2] bg-[#f8fbfd] px-3 py-2 text-xs font-black text-[#173a63] sm:flex-none"><Pencil size={14} /> تعديل</a>}
                    {profile.role === 'reviewer' && item.status === 'review' && <a href="/admin/review" className="inline-flex min-h-10 flex-1 items-center justify-center gap-1 rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800 ring-1 ring-amber-200 sm:flex-none"><ShieldCheck size={14} /> مراجعة</a>}
                    {canPublish && item.status === 'approved' && <button type="button" disabled={busyId === item.id} onClick={() => void publishApproved(item)} className="inline-flex min-h-10 flex-1 items-center justify-center gap-1 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-50 sm:flex-none"><Send size={14} /> نشر</button>}
                  </div>
                </article>
              ))}
            </div>
