...
export default function AdminEquipmentEditorPage() {
  const [profile, setProfile] = useState<AdminProfile | null>(null);
  const [row, setRow] = useState<EquipmentContentRow | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (id) {
        ...
        else if (rowData) setRow(rowData as EquipmentContentRow);
        else setError('الجهاز المطلوب غير موجود.');
      }
  ...
  if (!ready) {
    return <div className="flex min-h-screen items-center justify-center bg-[#07182c] text-sm font-bold text-white">جاري فتح محرر المعدات…</div>;
  }
