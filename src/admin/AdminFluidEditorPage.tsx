...
export default function AdminFluidEditorPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      if (profileError || !profileData?.is_active) { setError('هذا الحساب لا يملك صلاحية إدارة دليلي.'); setReady(true); return; }
  ...
  return <div className="admin-drug-editor-host"><FluidEditor profileId={profile.id} role={profile.role} initialRow={row} onSaved={() => undefined} onClose={() => window.location.assign('/admin')} /><MobileSelectSheet /></div>;
}
