...
export function AnesthesiaStagesDirectory({ initialQuery = '' }: { initialQuery?: string }) {
  ...
  const visibleStages = ANESTHESIA_STAGES
    .map((stage) => {
      const allGuides = getStageGuides(stage);
      if (!normalized) return { stage, guides: allGuides };
  ...
  return (
    <div className="space-y-3">
      <section className="rounded-[20px] border border-[#E5DCEF] bg-[#FAF8FC] px-4 py-4 text-right">
        ...
        {normalized && (
          <p className="mt-2 text-[10px] font-bold text-[#6C4AA5]">
            النتيجة المطابقة للبحث
          </p>
        )}
      </section>

      <div className="space-y-2.5">
        {visibleStages.map(({ stage, guides }) => (
          <details
            key={stage.id}
            open={Boolean(normalized) || undefined}
      ...
      {visibleStages.length === 0 && (
        <div className="rounded-[18px] border border-dashed border-[#D8CBE5] bg-[#FAF8FC] p-7 text-center text-xs text-[#81758A]">
          ماكو مرحلة أو موضوع مطابق للبحث.
        </div>
      )}
    </div>
  );
